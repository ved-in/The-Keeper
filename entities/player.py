...
def draw(screen, p):
    ...
    if p.get("target_world_x") is not None:
        ...
        # Animate pointer up/down with sine wave: oscillates between -height and +height
        # with period determined by p_speed (0.005 rad/ms gives ~6.28s full cycle).
    ...
    if frame:
        ...
        pygame.draw.rect(
            screen, (220, 200, 170), view.rect(p["x"], p["y"], p["w"], p["h"])
        )
